...
class editContract extends Component{
    constructor(props) {
        super(props);
        let id_contract = props.dataKey; // just for clarity
        let contract = props.getContract(id_contract);
        this.state= {
            formperteneceA:contract.perteneceA,
            formHorasCompradas:contract.horasCompradas,
    ...
    handleSubmit = (event) =>{
        ...
        fetch( serverURL+"/update/contrato",
        {
            method: "post",
...
const mapStateToProps = state =>{
    return{
        ...
        getContract : (id_contract) =>{
            return state.contracts.filter(contract => {return contract.numeroCorrelativo ==id_contract }).pop()    
